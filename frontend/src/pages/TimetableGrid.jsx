...
const TimetableGrid = () => {
  ...
  const buildGaSubjects = () => {
    // Theory subjects from mappings
    const subjects = (data.mappings || []).map(m => ({
      name: m.subjectName,
      faculty: m.faculty?.name || 'TBA',
      type: 'Theory',
      frequency: 5
    }));
    // Lab subjects from labMappings
    (data.labMappings || []).forEach(lm => {
      subjects.push({
        name: lm.lab?.name || 'Lab',
        faculty: lm.faculty?.name || 'TBA',
        type: 'Lab',
        frequency: 1  // Labs are 1 block (3 consecutive periods)
      });
    });
    return subjects;
  };
  ...
  const handleSemiGA = async () => {
    // ── Step 1: Show subject summary popup with editable frequencies ──
    const frozenSubjects = buildSubjectSummary();
    const gaSubjects = buildGaSubjects();
    const popupResult = await new Promise(resolve => {
      setSubjectSummary({
        frozenSubjects,
        gaSubjects,
        onConfirm: (frequencies) => { setSubjectSummary(null); resolve(frequencies); },
        onCancel:  () => { setSubjectSummary(null); resolve(null); }
      });
    });
    if (!popupResult) return;
    const subjectFrequencies = popupResult; // { subjectName: frequency }
